...
impl Blocker {
    ...
    pub(crate) fn replace(&self) -> NetworkFilterList<'_> {
        self.get_list(NetworkFilterListId::Replace)
    }
    ...
    pub fn get_replace_directives(&self, request: &Request) -> Vec<String> {
        let mut regex_manager = self.borrow_regex_manager();
        let filters =
            self.replace()
                .check_all(request, &self.tags_enabled, &mut regex_manager);
        ...
        for filter in filters {
            if filter.filter_mask.is_exception() {
                if let Some(directive) = filter.modifier_option {
                    disabled.insert(directive);
                } else {
                    return Vec::new();
                }
            } else if let Some(directive) = filter.modifier_option {
                enabled.insert(directive);
            }
        }

        enabled.difference(&disabled).cloned().collect()
    }
