...
impl AbstractNetworkFilter<'_> {
    pub(crate) fn parse<'a>(
        ...
        let replace_options_index = line.find("$replace").or_else(|| {
            line.find(",replace").and_then(|replace_index| {
                find_char_reverse(b'$', &line.as_bytes()[..replace_index])
            })
        });
        let maybe_options_index: Option<usize> =
            replace_options_index.or_else(|| find_char_reverse(b'$', line.as_bytes()));
...
fn parse_filter_options<'a>(
    ...
    for raw_option in split_filter_options(raw_options) {
        ...
        result.push(match (option, negation) {
            ...
            ("replace", true) => return Err(NetworkFilterError::NegatedReplace),
            ("replace", false) => NetworkFilterOption::Replace(if !value.is_empty() {
                Some(value)
            } else {
                None
            }),
...
fn split_filter_options(raw_options: &str) -> Vec<&str> {
    let bytes = raw_options.as_bytes();
    ...
    for (index, byte) in bytes.iter().enumerate() {
        ...
        if preceding_backslashes % 2 == 0 {
            parts.push(&raw_options[start..index]);
            start = index + 1;
        }
    }

    parts.push(&raw_options[start..]);
    parts
}
