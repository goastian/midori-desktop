...
// Midori Memory Profile settings
Preferences.addSetting({
  id: "memoryProfile",
  pref: "midori.memory.profile",
  get(val) {
    // Return current value or default to 1 (balanced)
    return val !== undefined ? val : 1;
  },
  set(val) {
    // Apply the memory profile when changed
    try {
      MemoryProfileManager.applyProfile(val);
    } catch (e) {
      console.error("Failed to apply memory profile:", e);
    }
    return val;
  },
});
...
let SETTINGS_CONFIG = {
  ...
  performance: {
    items: [
      ...
      {
        id: "memoryProfile",
        l10nId: "performance-memory-profile",
        control: "moz-box-item",
        supportPage: "performance",
      },
    ],
...
var gMainPane = {
  ...
  init() {
    ...
    // Midori Memory Profile initialization - delay to wait for template clone
    window.addEventListener("pageshow", () => this.initMemoryProfileUI(), { once: true });

    this.setInitialized();
  },

  /**
   * Initialize the Memory Profile UI
   */
  initMemoryProfileUI() {
    let menulist = document.getElementById("memoryProfileMenulist");
    if (!menulist) {
      // Try again after a short delay if element not found
      setTimeout(() => {
        menulist = document.getElementById("memoryProfileMenulist");
        if (menulist) {
          this._setupMemoryProfileMenulist(menulist);
        }
      }, 100);
      return;
    }
  ...
  /**
   * Setup the memory profile menulist
   */
  _setupMemoryProfileMenulist(menulist) {
    // Set initial value from preference
    let currentProfile = Services.prefs.getIntPref("midori.memory.profile", 1);
    menulist.value = String(currentProfile);

    // Add change listener
    menulist.addEventListener("command", event => {
      let newProfile = parseInt(event.target.value, 10);
      try {
        MemoryProfileManager.applyProfile(newProfile);
      } catch (e) {
        console.error("Failed to apply memory profile:", e);
      }
    });
