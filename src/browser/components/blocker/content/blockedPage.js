...
/*
 * The blocked URL is read from this page's query string.
 * The "Load anyway" click is delegated to the MidoriBlockedPage actor,
 * which records a permission for the session in the parent before
 * navigating.
 */

/**
 * @returns {{blockedUrl: string}}
 */
function parseState() {
  try {
...
const BLOCKED_PAGE_UNAVAILABLE_L10N_ID = "midori-blocked-page-unavailable";
...
function initPage() {
  ...
  setText(
    "blocked-url",
    validBlockedUrl ? state.blockedUrl : "",
  ...
  // Navigation runs in MidoriBlockedPageChild after the parent records a
