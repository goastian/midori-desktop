...
const { MAX_CUSTOM_FILTER_LISTS } = ChromeUtils.importESModule(
...
var gMidoriBlockerCustomFilterLists = {
  _createListItem(url) {
  ...
  _loadUrls() {
    ...
    for (const entry of parseUrlPref(raw)) {
      ...
      if (normalized) {
        ...
        if (this._urls.size >= MAX_CUSTOM_FILTER_LISTS) {
          break;
        }
      }
    }
  ...
  addUrl() {
    ...
    if (
      !this._urls.has(normalized) &&
      this._urls.size >= MAX_CUSTOM_FILTER_LISTS
    ) {
      document.l10n
        .formatValues([
          { id: "midori-blocker-custom-list-limit-title" },
          {
            id: "midori-blocker-custom-list-limit-message",
            args: { count: MAX_CUSTOM_FILTER_LISTS },
          },
        ])
        .then(([title, message]) => {
          Services.prompt.alert(window, title, message);
...
document.addEventListener("DOMContentLoaded", () => {
  gMidoriBlockerCustomFilterLists.init();
});
