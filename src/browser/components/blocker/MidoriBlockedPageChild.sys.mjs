 ...
 */
export class MidoriBlockedPageChild extends JSWindowActorChild {
  handleEvent(event) {
    ...
    this.sendQuery("MidoriBlockedPage:AllowAndNavigate", {
      url: blockedUrl,
    })
      .then(ok => {
        if (!ok) {
      ...
      .catch(err => {
        console.error(
          "[MidoriBlockedPageChild] AllowAndNavigate failed:",
          err
        );
      });
  }
