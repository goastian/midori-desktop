...
ChromeUtils.defineLazyGetter(lazy, "blockerLocalization", () => {
  return new Localization(["browser/midori/blocker.ftl"]);
});

const PREF_BLOCKER_ENABLED = "midori.blocker.enabled";
const PREF_BLOCKER_UI_ENABLED = "midori.blocker.ui.enabled";
...
const PREF_COEXIST = "midori.blocker.coexist";

const DETECTION_DELAY_MS = 500;
const CHROME_DOCUMENT_LOADED_TOPIC = "chrome-document-loaded";
const PREF_CHANGED_TOPIC = "nsPref:changed";
const EXTENSION_NAME_PLACEHOLDER = "__MIDORI_BLOCKER_EXTENSION_NAME__";
const L10N_ID_EXTENSION_FALLBACK_NAME_THIS =
  "midori-blocker-extension-fallback-name-this";
const L10N_ID_EXTENSION_FALLBACK_NAME_YOUR =
  "midori-blocker-extension-fallback-name-your";
const L10N_ID_INSTALL_WARNING = "midori-blocker-extension-install-warning";
const L10N_ID_INSTALL_WARNING_MANAGE_SETTINGS =
  "midori-blocker-extension-install-manage-settings";
const L10N_ID_PROMPT_TITLE = "midori-blocker-prompt-title";
const L10N_ID_INSTALL_ANYWAY = "midori-blocker-extension-install-anyway";
 ...
 * warnings read from.
 */
export const MidoriBlockerExtensionDetector = {
  _addonListener: {
    onEnabled(addon) {
      MidoriBlockerExtensionDetector._onAddonStateChanged(addon);
    },
    onDisabled(addon) {
      MidoriBlockerExtensionDetector._onAddonStateChanged(addon);
    },
  ...
  _installListener: {
    onInstallStarted(install) {
      return MidoriBlockerExtensionDetector._onInstallStarted(install);
    },
    onInstallEnded(install) {
      MidoriBlockerExtensionDetector._onInstallEnded(install);
    },
  },
  ...
  async _preloadLocalizedStrings() {
    ...
    const loadPromise = (async () => {
      ...
      this._localizedStringCache.set(
        "midori-blocker-extension-install-warning-template",
  ...
  init() {
    ...
    Services.tm.idleDispatchToMainThread(() => {
      ...
      this._preloadLocalizedStrings().catch(error => {
        console.error(
          "[MidoriBlockerExtensionDetector] Failed to preload localized strings",
          error
        );
      });
      this._refreshDetectionNotifications().catch(error => {
        console.error(
          "[MidoriBlockerExtensionDetector] Startup detection failed",
          error
        );
      });
    });
  ...
  uninit() {
    ...
    try {
      ...
      console.warn(
        "[MidoriBlockerExtensionDetector] Failed to remove enabled observer:",
        err
      );
    }
    ...
    try {
      ...
      console.warn(
        "[MidoriBlockerExtensionDetector] Failed to remove dismissed observer:",
        err
      );
    }
    ...
    try {
      ...
      console.warn(
        "[MidoriBlockerExtensionDetector] Failed to remove coexist observer:",
        err
      );
    }
  },
  ...
  _onAddonStateChanged(addon) {
    ...
    this._refreshDetectionNotifications().catch(error => {
      console.error(
        "[MidoriBlockerExtensionDetector] Failed to refresh detection notifications",
        error
      );
    });
  },
  ...
  _onBuiltInBlockerPrefChanged() {
    ...
    this._handleBuiltInBlockerReenabled()
      ...
      .catch(error => {
        console.error(
          "[MidoriBlockerExtensionDetector] Re-enable handling failed",
          error
        );
      });
  },
  ...
  _onDetectionDismissedPrefChanged() {
    ...
    this._refreshDetectionNotifications().catch(error => {
      console.error(
        "[MidoriBlockerExtensionDetector] Failed to re-enable detection notifications",
        error
      );
    });
  },
  ...
  _onCoexistPrefChanged() {
    ...
    this._refreshDetectionNotifications().catch(error => {
      console.error(
        "[MidoriBlockerExtensionDetector] Failed to refresh after coexist toggle",
        error
      );
    });
  },
  ...
  _removeStartupObserver() {
    ...
    try {
      ...
      console.warn(
        "[MidoriBlockerExtensionDetector] Failed to remove startup observer:",
        err
      );
    }
    ...
    const progressListener = {
      onLocationChange: (browser, webProgress, _request, location) => {
        ...
        this._maybeShowDetectionUpgradeForBrowser(win, browser, location).catch(
          error => {
            console.error(
              "[MidoriBlockerExtensionDetector] Failed to show upgrade modal",
              error
            );
          }
        );
  ...
  _seedDetectionForWindow(win, source = "seed") {
    ...
    const tryBrowser = browser => {
      this._maybeShowDetectionUpgradeForBrowser(
        ...
        console.error(
          `[MidoriBlockerExtensionDetector] Failed to show ${source} upgrade modal`,
          error
        );
      });
    };
  ...
  async _prewarmUpgradeMessage() {
    ...
    this._upgradeMessagePromise =
      lazy.OnboardingMessageProvider.getUpgradeMessage()
        ...
        .catch(error => {
          console.error(
            "[MidoriBlockerExtensionDetector] Failed to prewarm upgrade message",
            error
          );
          return null;
        })
  ...
  async _showDetectionUpgradeModal(win, browser, extensionName) {
    ...
    const [spotlightTitle, spotlightPrimaryLabel, spotlightSecondaryLabel] =
      await lazy.blockerLocalization.formatValues([
        "midori-blocker-spotlight-title",
        "midori-blocker-spotlight-primary-button",
        "midori-blocker-spotlight-secondary-button",
      ]);
    const detectedName =
      extensionName ||
    ...
    const spotlightSubtitle = await lazy.blockerLocalization.formatValue(
      "midori-blocker-spotlight-subtitle",
  ...
  _onInstallStarted(install) {
    ...
    this._preloadLocalizedStrings().catch(error => {
      console.error(
        "[MidoriBlockerExtensionDetector] Failed to refresh install warning strings",
        error
      );
    });
  ...
  async _disableAdblockExtensions(addons) {
    ...
    for (const addon of addons) {
      try {
        ...
        failed = true;
        console.error(
          `[MidoriBlockerExtensionDetector] Failed to disable ${addon?.id || "<unknown>"}`,
          error
        );
      }
    }
  ...
  async _handleBuiltInBlockerReenabled() {
    ...
    const [promptTitle, messageText, useBuiltInLabel, keepExtensionLabel] =
      await lazy.blockerLocalization.formatValues([
        "midori-blocker-prompt-title",
        {
          id: "midori-blocker-reenable-conflict-message",
          args: { extensionName: conflictingName },
        },
        "midori-blocker-reenable-use-built-in",
        "midori-blocker-reenable-keep-extension",
      ]);
  ...
  _showInstallWarning(win, addon) {
    try {
      ...
      console.error(
        "[MidoriBlockerExtensionDetector] Failed to show install warning prompt",
        error
      );
      return true;
    }
