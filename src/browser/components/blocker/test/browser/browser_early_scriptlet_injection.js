...
async function readProbeState(browser) {
  return SpecialPowers.spawn(browser, [], () => {
    ...
    return {
      diagnostics: win.__midoriBlockerEarlyScriptletDiagnostics || null,
...
add_setup(function setup() {
  originalGetCosmeticResources = MidoriBlockerService.getCosmeticResources;
  originalInitializeEngineWithRetry =
    MidoriBlockerService._initializeEngineWithRetry;
  originalWhenEngineReady = MidoriBlockerService.whenEngineReady;

  MidoriBlockerService._initializeEngineWithRetry = async function () {};
  MidoriBlockerService.getCosmeticResources = function (url) {
  ...
  MidoriBlockerService.whenEngineReady = async function () {
  ...
  registerCleanupFunction(() => {
    MidoriBlockerService.getCosmeticResources = originalGetCosmeticResources;
    MidoriBlockerService._initializeEngineWithRetry =
      originalInitializeEngineWithRetry;
    MidoriBlockerService.whenEngineReady = originalWhenEngineReady;
...
add_task(async function test_latency_diagnostics() {
  await withDiagnostics(async () => {
    ...
    info(
      `MidoriBlocker early parser block median no-scriptlet duration: ${median(
        noScriptletDurations
      )}ms`
    );
    info(
      `MidoriBlocker early parser block median scriptlet duration: ${median(
        scriptletDurations
      )}ms`
    );
  }, [earlyScriptletTimeoutPref()]);
