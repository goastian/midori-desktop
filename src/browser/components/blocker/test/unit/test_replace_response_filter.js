...
add_setup(async function setup() {
  ...
  MidoriBlockerService._registerNetworkObservers();

  registerCleanupFunction(async () => {
    MidoriBlockerService._unregisterNetworkObservers();
...
add_task(async function test_replace_response_filter_respects_site_bypass() {
  makeEngine();
  const originalBypass = MidoriBlockerService.shouldBypassBlocking;
  MidoriBlockerService.shouldBypassBlocking = (candidateDomain, options) =>
    candidateDomain === "127.0.0.1" ||
    originalBypass.call(MidoriBlockerService, candidateDomain, options);
  ...
  try {
    ...
    MidoriBlockerService.shouldBypassBlocking = originalBypass;
  }
});
