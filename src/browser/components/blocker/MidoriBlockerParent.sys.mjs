 ...
 */
export class MidoriBlockerParent extends JSWindowActorParent {
  /**
  ...
  receiveMessage(message) {
    switch (message.name) {
      case "MidoriBlocker:GetCosmeticResources":
        return this._getCosmeticResources(message.data);
      case "MidoriBlocker:GetHiddenClassIdSelectors":
  ...
  async _getCosmeticResources({ url } = {}) {
    ...
    await MidoriBlockerService.whenEngineReady();

    const resources = MidoriBlockerService.getCosmeticResources(
      url,
  ...
  _getHiddenClassIdSelectors({ classes, ids, exceptions } = {}) {
    return MidoriBlockerService.getHiddenClassIdSelectors(
      classes || [],
      ids || [],
      exceptions || []
    );
