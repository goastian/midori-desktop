...
const MIDORI_UNBREAK_FILTERS_FILE_NAME = "midori-unbreak.txt";
const MIDORI_UNBREAK_FILTERS_URL =
  "resource://midori/blocker/assets/filters/midori-unbreak.txt";

let gMidoriUnbreakRecord = null;
...
async function readCustomFiltersRecord(customDescriptor) {
  ...
  try {
    ...
    console.warn("[MidoriBlocker] Failed reading custom filters:", err);
    return null;
  }
}

async function readMidoriUnbreakRecord() {
  if (gMidoriUnbreakRecord) {
    return gMidoriUnbreakRecord;
  }

  const response = await fetch(MIDORI_UNBREAK_FILTERS_URL, {
    cache: "no-store",
  });
  if (!response.ok) {
  ...
  if (!text.trim()) {
    throw new Error("Midori unbreak filters were empty");
  }

  gMidoriUnbreakRecord = {
    filename: MIDORI_UNBREAK_FILTERS_FILE_NAME,
    text,
    url: MIDORI_UNBREAK_FILTERS_URL,
  };
  return gMidoriUnbreakRecord;
}

async function withMidoriUnbreakRecord(listRecords) {
  return [...listRecords, await readMidoriUnbreakRecord()];
}
    ...
    for (const descriptor of descriptors) {
      ...
      for (const filename of filenames) {
        ...
        try {
          ...
          console.warn(
            `[MidoriBlocker] Failed reading stored list ${filename}:`,
            err
          );
        }
      }
  ...
  async readBundledLists(descriptors) {
    ...
    for (const descriptor of descriptors) {
      ...
      try {
        ...
        console.warn(
          `[MidoriBlocker] Failed to read bundled list: ${descriptor.bundledUrl}`,
          err
        );
      }
    }
