...
import { CUSTOM_FILTERS_FILE_NAME } from "resource:///modules/MidoriBlockerUtils.sys.mjs";

export const CUSTOM_FILTERS_DESCRIPTOR_URL = "midori://custom-filters";
...
export const MAX_CUSTOM_FILTER_LISTS = 32;

const LIST_CATALOG_URL = "resource://midori/blocker/assets/list_catalog.json";
const BUNDLED_FILTERS_BASE = "resource://midori/blocker/assets/filters/";
...
function getCustomFilterListUrls() {
  ...
  for (const entry of entries) {
    ...
    seen.add(href);
    urls.push(href);
    if (urls.length >= MAX_CUSTOM_FILTER_LISTS) {
      break;
    }
  }
  ...
  if (entries.length && !raw.trim().startsWith("[")) {
    try {
      ...
      console.warn(
        "[MidoriBlocker] Failed migrating custom list URL pref:",
        err
      );
    }
  }
...
export const ListCatalog = {
  ...
  async getListDescriptors() {
    ...
    if (Services.prefs.getBoolPref(PREF_CUSTOM_FILTERS_ENABLED, true)) {
