...
ChromeUtils.defineESModuleGetters(lazy, {
  ...
  LegacyMigration: "resource:///modules/internal/LegacyMigration.sys.mjs",
...
const CONTRACT_ID = "@astian.org/midori-blocker-engine;1";
...
const PREF_LEGACY_SITE_EXCEPTIONS = "midori.blocker.siteExceptions";
const PREF_SITE_EXCEPTIONS_MIGRATED =
  "midori.blocker.siteExceptions.migrated";
const PREF_REMOTE_RESOURCES_ENABLED = "midori.blocker.remoteResourcesEnabled";
const PREF_GLOBAL_STATS = "midori.blocker.globalStats";
const PREF_DOMAIN_EXCEPTIONS = "midori.blocker.domainExceptions";
...
const MAX_ENGINE_FILTER_BYTES = 64 * 1024 * 1024;
...
function applyReplaceDirectives(text, directives) {
  ...
  for (const directive of directives) {
    try {
      ...
      console.warn(
        "[MidoriBlocker] Skipping invalid $replace directive:",
        err
      );
    }
  }
...
function applyHtmlFilters(text, filters, documentUrl, mimeType) {
  ...
  for (const rawFilter of filters) {
    ...
    try {
      ...
      console.warn("[MidoriBlocker] Skipping invalid HTML filter:", err);
      continue;
    }
  ...
  onStopRequest(request, statusCode) {
    if (!this.passThrough) {
      ...
      try {
        ...
        console.warn("[MidoriBlocker] Failed to filter response body:", err);
        this.emitBytes(request, originalBytes);
      }
    }
 ...
 */
export const MidoriBlockerService = {
  QueryInterface: ChromeUtils.generateQI([
    "nsIObserver",
  ...
  _pendingStartupChannels: new Set(),
  _initGeneration: 0,
  ...
  _createEngine() {
    return Cc[CONTRACT_ID].createInstance(Ci.nsIMidoriBlockerEngine);
  },
  ...
  async _preprocessListRecords(listRecords) {
    const records = await lazy.ListStore.withMidoriUnbreakRecord(listRecords);
  ...
  _createEngineFromListRecords(listRecords) {
    ...
    for (const record of listRecords) {
      ...
      if (totalBytes > MAX_ENGINE_FILTER_BYTES) {
        throw new Error(
          `Filter list input exceeds ${MAX_ENGINE_FILTER_BYTES} bytes`
        );
      }
      filterLists.push(text);
    }
    ...
    const engine = this._createEngine();
    engine.initFromLists(filterLists);
    return engine;
  },
  ...
  async _doInitializeEngineIfNeeded() {
    ...
    try {
      ...
      if (e.result !== Cr.NS_ERROR_FILE_NOT_FOUND) {
        console.error("[MidoriBlocker] Unexpected cache error:", e);
      }
    }
  ...
  _notifyBlockedCountsCleared() {
    try {
      ...
      console.warn("[MidoriBlocker] Failed to notify cleared counts:", err);
    }
  },
  ...
  _notifyBlockedCountUpdated(browserId, blockedCount) {
    try {
      ...
      console.warn("[MidoriBlocker] Failed to notify blocked count:", err);
    }
  },

  _buildBlockedPageUrl(url) {
  ...
  _handleTopLevelDocumentRequest(
    ...
    try {
      ...
      console.warn("[MidoriBlocker] Failed to increment blocked count:", err);
    }
  },
  ...
  _onModifyRequest(subject) {
    ...
    if (!this._engine) {
      const readyPromise = this._startupPromise || this._engineInitPromise;
      if (readyPromise) {
        this._deferChannelUntilReady(channel, readyPromise);
      }
      return;
    }
    ...
    if (result.matched && !result.exception) {
      ...
      try {
        ...
        console.warn(
          "[MidoriBlocker] Failed to increment blocked count:",
          err
        );
      }
      return;
  ...
  _deferChannelUntilReady(channel, readyPromise) {
    if (this._pendingStartupChannels.has(channel)) {
      return;
    }
    ...
    this._pendingStartupChannels.add(channel);
    Promise.resolve(readyPromise)
      .catch(() => {})
      .finally(() => {
        if (!this._pendingStartupChannels.delete(channel)) {
          return;
        }
        if (this._initialized && this.isEnabled() && this._engine) {
          this._onModifyRequest(channel);
        }
        try {
          channel.resume();
        } catch (_) {}
  ...
  _resumePendingStartupChannels() {
    for (const channel of this._pendingStartupChannels) {
      try {
        channel.resume();
      } catch (_) {}
    }
    this._pendingStartupChannels.clear();
  },
  ...
  _redirectChannel(channel, target) {
    try {
      ...
      console.warn("[MidoriBlocker] Failed to redirect channel:", err);
      return false;
    }
  },
  ...
  _onExamineResponse(subject) {
    ...
    try {
      ...
      console.error("[MidoriBlocker] Failed to apply CSP directives:", err);
    }
  },
  ...
  _maybeFilterResponseBody(
    ...
    try {
      ...
      console.warn("[MidoriBlocker] Failed to install response filter:", err);
    }
  },
  ...
  async refreshListsAndEngine({ forceUpdate = false } = {}) {
    await this._rebuildEngineFromCurrentSources({
      preservePreviousEngine: true,
    });
    await this._updateListsIfNeeded({ force: forceUpdate });
  },
  ...
  _startListUpdateTriggers() {
    ...
    this._fallbackUpdateIntervalId = lazy.setInterval(() => {
      this._updateListsIfNeeded().catch(err => {
        console.warn("[MidoriBlocker] Fallback list update failed:", err);
      });
  ...
  _stopListUpdateTriggers() {
    if (this._listUpdateObserverRegistered) {
      try {
        ...
        console.warn(
          "[MidoriBlocker] Failed to remove RS poll observer:",
          err
        );
      }
  ...
  async _refreshEngineAfterListUpdate(anyUpdated, descriptors, generation) {
    ...
    if (anyUpdated) {
      ...
      if (!complete) {
        ...
        console.warn(
          `[MidoriBlocker] ${missingCount} bundled filter list(s) unavailable after update; keeping the current engine`
        );
  ...
  async _runListUpdatePass({ force = false } = {}) {
    ...
    try {
      await lazy.RemoteResources.refresh({ force });
    } catch (err) {
      console.warn("[MidoriBlocker] Remote resource refresh failed:", err);
    }
  ...
  async _updateListsIfNeeded({ force = false } = {}) {
    ...
    const promise = (async () => {
      let forceNextPass = force;
      do {
        this._listUpdateRerunRequested = false;
        const forceThisPass =
          forceNextPass || this._listUpdateForceRequested;
        forceNextPass = false;
        this._listUpdateForceRequested = false;
        await this._runListUpdatePass({ force: forceThisPass });
      } while (
        this._listUpdateRerunRequested &&
        this._initialized &&
        this.isEnabled()
  ...
  _parseFastCheckResult(rawResult) {
    switch (rawResult) {
      case "":
        return this._normalizeCheckResult(null);
      case "B":
        return this._normalizeCheckResult({ matched: true });
      case "I":
        return this._normalizeCheckResult({
          important: true,
          matched: true,
        });
      case "E":
        return this._normalizeCheckResult({ exception: true });
      default:
        return this._normalizeCheckResult(JSON.parse(rawResult));
    }
  },
  ...
  checkRequest(
    ...
    try {
      ...
      if (typeof this._engine.checkRequestFast === "function") {
        return this._parseFastCheckResult(
          this._engine.checkRequestFast(...args)
        );
      }
      return this._normalizeCheckResult(
        JSON.parse(this._engine.checkRequestDetailed(...args))
      );
    } catch (err) {
      console.error("[MidoriBlocker] checkRequest failed:", err);
      return this._normalizeCheckResult(null);
    }
  },
  ...
  getCosmeticResources(url, browsingContext = null) {
    ...
    try {
      ...
      console.error("[MidoriBlocker] getCosmeticResources failed:", err);
      return null;
    }
  ...
  getCspDirectives(
    ...
    try {
      ...
      console.error("[MidoriBlocker] getCspDirectives failed:", err);
      return "";
    }
  },
  ...
  getReplaceDirectives(
    ...
    try {
      ...
      console.error("[MidoriBlocker] getReplaceDirectives failed:", err);
      return [];
    }
  },
  ...
  getHtmlFilteringResources(url) {
    ...
    try {
      ...
      console.error("[MidoriBlocker] getHtmlFilteringResources failed:", err);
      return { htmlFilters: [], responseHeaderFilters: [] };
    }
  },
  ...
  areCustomFiltersEnabled() {
    return Services.prefs.getBoolPref(PREF_CUSTOM_FILTERS_ENABLED, true);
  },
   ...
   * @param {string} text
   */
  async setCustomFiltersText(text, options = {}) {
    ...
    const previousEnabled = this.areCustomFiltersEnabled();
    const enabled =
      typeof options.enabled === "boolean" ? options.enabled : previousEnabled;
    let previousText = "";
    ...
    if (hadPreviousFile) {
      try {
        ...
        console.warn(
          "[MidoriBlocker] Failed reading previous custom filters for rollback:",
          err
        );
      }
    }
    ...
    Services.prefs.setBoolPref(PREF_CUSTOM_FILTERS_ENABLED, enabled);
    ...
    try {
      ...
      try {
        ...
        console.error(
          "[MidoriBlocker] Failed rolling back custom filters after rebuild failure:",
          rollbackErr
        );
      }
  ...
  getHiddenClassIdSelectors(classes = [], ids = [], exceptions = []) {
    ...
    try {
      ...
      console.error("[MidoriBlocker] getHiddenClassIdSelectors failed:", err);
      return [];
    }
  },
  ...
  _recordBlockedRequestStats(browserId, details) {
    ...
    const category = details?.topLevel
      ? "popups"
      : TRACKER_REQUEST_TYPES.has(String(details?.requestType || ""))
        ? "trackers"
        : "ads";
    ...
    if (domain) {
      ...
      if (entry) {
        ...
        if (category === "ads") {
          ...
          entry.unclassifiedCount++;
          entry.hostname = this._normalizeHostname(details?.hostname);
        }
      } else if (stats.domains.size < BLOCKED_DOMAINS_PER_TAB_MAX) {
        stats.domains.set(domain, {
          category,
          classified: category !== "ads",
          count: 1,
          hostname: this._normalizeHostname(details?.hostname),
          unclassifiedCount: category === "ads" ? 1 : 0,
        });
      }
    }
  },

  async prepareBlockedStats(browserId) {
    const id = Number(browserId || 0);
    const stats = this._blockedStatsByBrowserId.get(id);
    if (!stats) {
      return;
    }
    if (stats.classificationPromise) {
      return stats.classificationPromise;
    }

    const entries = [...stats.domains.values()].filter(
      entry => !entry.classified && entry.hostname
    );
    if (!entries.length) {
      return;
    }
    ...
    const promise = Promise.all(
      entries.map(async entry => {
        const category =
          (await this._classifyDomainViaTrackingTables(entry.hostname)) ||
          "ads";
        if (this._blockedStatsByBrowserId.get(id) !== stats) {
          return;
        }
        const reclassifiedCount = entry.unclassifiedCount;
        if (category !== "ads" && reclassifiedCount) {
          stats.counts.ads -= reclassifiedCount;
          stats.counts[category] += reclassifiedCount;
          entry.category = category;
        }
        entry.classified = true;
        entry.unclassifiedCount = 0;
      })
    ).then(() => {
      if (this._blockedStatsByBrowserId.get(id) === stats) {
    ...
    stats.classificationPromise = promise.finally(() => {
      if (this._blockedStatsByBrowserId.get(id) === stats) {
        stats.classificationPromise = null;
      }
    });
    return stats.classificationPromise;
  },
  ...
  _flushGlobalStats() {
    ...
    try {
      ...
      console.warn("[MidoriBlocker] Failed to persist global stats:", err);
    }
  },
  ...
  _saveDomainExceptions() {
    ...
    try {
      ...
      console.warn(
        "[MidoriBlocker] Failed to persist domain exceptions:",
        err
      );
    }
  },
  ...
  _unregisterNetworkObservers() {
    ...
    for (const topic of [
      ...
      try {
        ...
        console.warn(
          `[MidoriBlocker] Failed to remove observer for ${topic}:`,
          err
        );
      }
    }
  ...
  async _initializeEngineWithRetry() {
    try {
      ...
      if (this.isEnabled()) {
        ...
        this._updateListsIfNeeded().catch(err => {
          console.warn("[MidoriBlocker] Initial list update failed:", err);
        });
      }
    } catch (err) {
      console.error("[MidoriBlocker] Failed to initialise engine:", err);
      this._scheduleInitRetry();
    }
  },
  ...
  _migrateSiteExceptions() {
    ...
    if (raw) {
      try {
        ...
        console.warn(
          "[MidoriBlocker] Failed to parse legacy site exceptions pref:",
          err
        );
      }
    }
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        console.warn(
          `[MidoriBlocker] Failed to migrate site exception "${entry}":`,
          err
        );
      }
    }
  ...
  init() {
    ...
    const startupPromise = (async () => {
      try {
        ...
        await lazy.LegacyMigration.migrate({
          listStore: lazy.ListStore,
          siteExceptions: this._siteExceptions(),
        });
        if (!this._initialized) {
          return;
        }
        ...
        lazy.EngineCache.cleanupStale().catch(err => {
          console.warn("[MidoriBlocker] Cache cleanup failed:", err);
        });
        await this._initializeEngineWithRetry();
      } catch (err) {
        this._unregisterNetworkObservers();
        this._resumePendingStartupChannels();
        this._initialized = false;
        throw err;
      }
    })();

    this._startupPromise = startupPromise.finally(() => {
  ...
  observe(subject, topic, data) {
    ...
    if (topic === REMOTE_SETTINGS_POLL_END_TOPIC) {
      this._updateListsIfNeeded().catch(err => {
        console.warn(
          "[MidoriBlocker] List update on RS poll-end failed:",
          err
        );
      });
      return;
    ...
    switch (data) {
      ...
      case PREF_FILTER_LIST_URLS:
        if (this.isEnabled()) {
          this.refreshListsAndEngine().catch(err => {
            console.error(
              "[MidoriBlocker] Failed to refresh lists after pref change:",
              err
            );
          });
        }
      ...
      case PREF_ENABLED_LISTS:
        if (this.isEnabled()) {
          this.refreshListsAndEngine().catch(err => {
            console.error(
              "[MidoriBlocker] Failed to refresh lists after list toggle:",
              err
            );
          });
        }
      ...
      case PREF_REMOTE_RESOURCES_ENABLED:
        if (this.isEnabled() && this._engine) {
          this._loadResourcesAndBumpGeneration().catch(err => {
            console.error(
              "[MidoriBlocker] Failed to reload resources after remoteResourcesEnabled change:",
              err
            );
          });
        }
  ...
  uninit() {
    ...
    try {
      ...
      console.warn("[MidoriBlocker] Failed to remove pref observer:", err);
    }
