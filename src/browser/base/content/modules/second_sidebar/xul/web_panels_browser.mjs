...
export class WebPanelsBrowser extends Browser {
  ...
  #hackCloseWindowCommand() {
    ...
    const elements = window.document.querySelectorAll('[command="cmd_closeWindow"]');
  ...
  initWindow() {
    ...
    const tabbox = windowRoot.querySelector("#tabbrowser-tabbox");
    tabbox.setProperty("height", "100%");
    
    // Ensure tabpanels are visible
    const tabpanels = windowRoot.querySelector("#tabbrowser-tabpanels");
    if (tabpanels) {
      tabpanels.setProperty("height", "100%");
      tabpanels.setProperty("display", "block");
    }
    
    // Ensure browsers are visible
    const browsers = windowRoot.querySelectorAll("browser[type='content']");
    browsers.forEach(browser => {
      browser.setProperty("height", "100%");
      browser.setProperty("width", "100%");
      browser.setProperty("display", "block");
    });
    ...
    // Create gURLBar stub to prevent errors in webpanel browser context
    if (!this.window.gURLBar) {
      console.log("Creating gURLBar stub for webpanel browser");
      this.window.gURLBar = {
        value: "",
        focused: false,
        getBrowserState: () => ({}),
