...
export class WebPanelsController {
  ...
  #setupListeners() {
    ...
    listenEvent(WebPanelEvents.CREATE_WEB_PANEL, async (event) => {
      ...
      if (temporary) {
        if (isActiveWindow) {
          ...
          window.setTimeout(() => this.#unwrapButtons(), 100);
        }
      } else {
        const webPanelController = await create();
        if (isActiveWindow) {
    ...
    listenEvent(WebPanelEvents.EDIT_WEB_PANEL_URL, (event) => {
      ...
      window.clearTimeout(this.urlTimeout);
    ...
    listenEvent(WebPanelEvents.EDIT_WEB_PANEL_FAVICON_URL, (event) => {
      ...
      window.clearTimeout(this.faviconURLTimeout);
      this.faviconURLTimeout = window.setTimeout(() => {
  ...
  loadSettingsAndState(webPanelsSettings, webPanelsState) {
    ...
    SidebarElements.webPanelsBrowser.waitInitialization(() => {
      ...
      console.log(`Loading ${webPanelsSettings.webPanels.length} web panels from settings`);
      for (const webPanelSettings of webPanelsSettings.webPanels) {
        const uuid = webPanelSettings.uuid;
        console.log(`Creating web panel: ${uuid}, URL: ${webPanelSettings.url}`);
        const webPanelState =
        ...
        this.add(webPanelController);
        console.log(`Web panel ${uuid} added to controller`);
      }
      // Don't show the sidebar automatically on startup
      // It will be shown when a webpanel is explicitly opened by the user
      console.log("Web panels loaded, sidebar ready but not shown");
    });
  }
