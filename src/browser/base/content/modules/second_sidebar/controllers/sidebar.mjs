...
export class SidebarController {
  ...
  open(webPanelController = null) {
    console.log("SidebarController.open() called");
    SidebarControllers.sidebarToolbarCollapser.clearTimers();

    // Get the webPanelController if not provided
    if (!webPanelController) {
      webPanelController = SidebarControllers.webPanelsController.getActive();
    }
    
    // Validate webPanelController exists
    if (!webPanelController) {
      console.error("SidebarController.open(): No active webPanelController found");
      return;
    }
  ...
  pin() {
    ...
    window.document.removeEventListener("click", this.onClickOutsideWhileFloating);
  }
  ...
  unpin() {
    ...
    window.document.addEventListener("click", this.onClickOutsideWhileFloating);
  }
  ...
  loadSettings(settings) {
    // Set initial pinned state (default to false for floating sidebar)
    const pinned = settings.pinned !== undefined ? settings.pinned : false;
    SidebarElements.sidebarBox.setAttribute("pinned", pinned);
    console.log("Sidebar pinned state set to:", pinned);
