...
export class WebPanelController {
    ...
    const onStateChange = (aWebProgress, aRequest, aFlag) => {
      ...
      if (
        ...
        if (this.getSelectorEnabled()) {
          window.setTimeout(() => this.#applySelector(), 100);
        }
      }
    };
    return {
  ...
  #applySelector() {
    ...
    const script = `javascript:(() => {
      var toDelete = [];
      var e = window.document.querySelector('${selector}');
      e.style.margin = 0;
  ...
  switchWebPanel({ forceOpen = false } = {}) {
    console.log(`switchWebPanel called for uuid: ${this.getUUID()}`);
    const activeTab = SidebarElements.webPanelsBrowser.getActiveWebPanelTab();
    console.log("Current active tab uuid:", activeTab.uuid);

    if (activeTab.uuid === this.getUUID() && !forceOpen) {
      console.log("Same tab, deselecting");
      // Select empty web panel tab
      SidebarElements.webPanelsBrowser.deselectWebPanelTab();
      SidebarControllers.sidebarController.close();
    } else {
      console.log("Different tab, selecting");
      // Create web panel tab if it was not loaded yet
      if (this.isUnloaded()) {
        console.log("Tab is unloaded, loading...");
        this.load();
      }
      // Select web panel tab
      console.log("Selecting web panel tab");
      SidebarElements.webPanelsBrowser.selectWebPanelTab(this.#tab);
      // Directly open the webpanel since TabSelect event may not fire
      console.log("Calling open() directly");
      this.open();
    }
  }
  ...
  open() {
    ...
    SidebarControllers.sidebarController.open(this);
  }
  ...
  load() {
    console.log(`WebPanelController.load() called for uuid: ${this.getUUID()}`);
    this.#tab = SidebarElements.webPanelsBrowser.addWebPanelTab(
      this.#settings,
      this.#progressListener,
    );
    console.log("Tab created:", this.#tab);
    console.log("Tab linkedBrowser:", this.#tab?.linkedBrowser);
   ...
   */
  go(url) {
    console.log(`WebPanelController.go() called with URL: ${url}`);
    console.log("Tab:", this.#tab);
    console.log("LinkedBrowser:", this.#tab?.linkedBrowser);
    if (this.#tab && this.#tab.linkedBrowser) {
      this.#tab.linkedBrowser.go(url);
      console.log("URL loaded successfully");
    } else {
      console.error("Cannot load URL: tab or linkedBrowser is null");
    }
  }
