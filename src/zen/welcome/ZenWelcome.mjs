...

{
  ...
  async function openInitialPinTab() {
    const tabs = [
      [
        'https://astian.org/midori-browser/wallet',
        'Midori Wallet',
  ...
  async function animateInitialStage() {
    const [title1, title2, title3] = await document.l10n.formatValues([
      { id: 'zen-welcome-title-line1' },
      { id: 'zen-welcome-title-line2' },
      { id: 'zen-welcome-title-line3' },
    ]);
    const titleElement = document.getElementById('zen-welcome-title');
    titleElement.innerHTML = `<html:span>${title1}</html:span><html:span>${title2}</html:span><html:span>${title3}</html:span>`;
    await animate(
