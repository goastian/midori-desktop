        ...
        var utils = {
          ...
          getDisabledMinuteArr: function (enableRanges, minuteStep) {
            ...
            function setDisabled(enableRange) {
              var beginDisabledMinute = Math.ceil(
                enableRange.begin / minuteStep,
              );
        ...
        var TimePicker = defineClass(
          /** @lends TimePicker.prototype */ {
            ...
            renderTimeInputs: function () {
              ...
              var hourElement = this.element.querySelector(
                SELECTOR_HOUR_ELEMENT,
              );
              var minuteElement = this.element.querySelector(
                SELECTOR_MINUTE_ELEMENT,
              );
              var BoxComponent =
                this.inputType.toLowerCase() === "selectbox"
                  ? Selectbox
                  : Spinbox;
            ...
            syncToMeridiemElements: function () {
              ...
              var notSelectedEl =
                selectedEl === this.pmEl ? this.amEl : this.pmEl;
            ...
            setRangeHour: function (beginHour, endHour) {
              var disabledHours = util.getRangeArr(
                START_NUMBER_OF_TIME,
                beginHour - 1,
              );

              if (endHour) {
                disabledHours = disabledHours.concat(
                  util.getRangeArr(endHour + 1, END_NUMBER_OF_HOUR),
                );
              }
            ...
            applyRange: function (beginHour, beginMin, endHour) {
              ...
              if (this.isLaterThanSetTime(beginHour, beginMin)) {
                if (this.disabledMinutes[targetHour][targetMinuteIndex]) {
                  ...
                  targetMinute =
                    diffFromSelectableMinute >= 0
                      ? targetMinute + diffFromSelectableMinute
                      : 60;
                }

                if (
                  (this.hourStep !== 1 && beginHour % this.hourStep !== 1) ||
                  targetMinute >= 60
                ) {
              ...
              if (this.showMeridiem) {
                ...
                util.setDisabled(
                  this.amEl,
                  beginHour >= END_NUMBER_OF_HOUR_WITH_MERIDIEM,
                );
                util.setDisabled(
                  this.pmEl,
                  endHour < END_NUMBER_OF_HOUR_WITH_MERIDIEM,
                );
              }
            },
            ...
            isValidRange: function (begin, end) {
              ...
              return (
                this.isValidTime(endHour, endMin) &&
                this.compareTimes(begin, end) > 0
              );
            },
            ...
            isLaterThanSetTime: function (hour, minute) {
              return (
                hour > this.hour || (hour === this.hour && minute > this.minute)
              );
            },
