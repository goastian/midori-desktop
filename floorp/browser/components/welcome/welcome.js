...
const designOptions = document.getElementsByClassName(
	"third-design-card-content",
);
...
const welcomeFunctions = {
	pageFunctions: {
		init() {
			...
			const importButton = document.getElementById(
				"import-data-floorp-welcome-button",
			);
			importButton.setAttribute(
				"onclick",
				"welcomeFunctions.manageBrowserFunctions.openMigrationWizard()",
			);

			// setting
			const defaultSettingButton = document.getElementById(
				"apply-default-setting",
			);
			const minimumSettingButton = document.getElementById(
				"apply-minimum-setting",
			);
			const maximumSettingButton = document.getElementById(
				"apply-maximum-setting",
			);
			defaultSettingButton.setAttribute(
				"onclick",
				"welcomeFunctions.manageBrowserFunctions.applyDefautlSettings()",
			);
			minimumSettingButton.setAttribute(
				"onclick",
				"welcomeFunctions.manageBrowserFunctions.applyMinimumSettings()",
			);
			maximumSettingButton.setAttribute(
				"onclick",
				"welcomeFunctions.manageBrowserFunctions.applyMaximumSettings()",
			);

			// add event listeners for Design selection
			for (let i = 0; i < designOptions.length; i++) {
				designOptions[i].setAttribute(
					"onclick",
					`welcomeFunctions.elementManageFunction.clickToSelectDesign(this, ${i})`,
				);
			}
	...
	manageBrowserFunctions: {
		openMigrationWizard() {
			Promise.all([MigrationUtils.showMigrationWizard(null, {})]).then(() => {
				welcomeFunctions.pageFunctions.clickNextButton();
			});
		},
