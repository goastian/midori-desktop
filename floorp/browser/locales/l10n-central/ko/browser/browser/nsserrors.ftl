...
ssl-error-no-compression-overlap = 피어와 안전하게 통신을 할 수 없음: 일반 압축 알고리듬이 없습니다.
...
ssl-error-inappropriate-fallback-alert = 클라이언트가 서버가 지원하는 것보다 낮은 TLS 버전으로 다운그레이드되었기 때문에 서버가 핸드셰이크를 거부했습니다.
