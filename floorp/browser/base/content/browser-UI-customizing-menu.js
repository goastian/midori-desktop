...
let gFloorpCustomizeMode = {
    addElemToCustomizeModeArea(elem, enabled, targetId, type, retry){
        if (retry > 3) {
            throw new Error(`Retry count is over. targetId: ${targetId}, type: ${type}`);
        }
        ...
        try {
            ...
            if (!button) {
                throw new Error(`targetElem is not found. targetId: ${targetId}, type: ${type}`);
            }
            switch (type) {
                case "before":
                    button.before(elem);
...
(function addToggleVisibleUnifiedExtensionButton() {
    ...
    />`);
    let enabled = !Services.prefs.getBoolPref("floorp.hide.unifiedExtensionsButtton", false);
    gFloorpCustomizeMode.addElemToCustomizeModeArea(elem, enabled, "customization-titlebar-visibility-checkbox", "after", 0);
})();