...
if (browser_.runtime.id == "{506e023c-7f2b-40a3-8066-bc5deb40aebe}") {
  let gesturefyController = {
    commandsId: [
      ...
      "open-synctabs-sidebar",
      "open-extension-sidebar",
      "close-sidebar",
      "open-browser-manager-sidebar",
      "close-browser-manager-sidebar",
      "toggle-browser-manager-sidebar",
      "show-statusbar",
      "hide-statusbar",
      "toggle-statusbar",
    ],
    commandSettings: {
        ...
        UI: function () {
          ...
          if (
            ...
            if (
              ...
              selectedExtension = JSON.parse(valueJSON.settings.message).options
                .extensionId;
            }
          } else {
            isInSidebar = true;
      ...
      <select class="select-field" id="open-extension-sidebar-extension" required>
        <option value="" hidden>${
          gesturefyController.l10n[
            isInSidebar
              ? `gf-floorp-open-extension-sidebar-settings-list-default`
              : `gf-floorp-open-extension-sidebar-settings-list-unknwon`
          ]
        }</option>
        ${extensionList}
      </select>
    ...
    makeSettingPage: function (id) {
      ...
      settingsHeading.title = settingsHeading.textContent =
        gesturefyController.l10n["gf-floorp-open-extension-sidebar-name"];

      const settingsContent = templateFragment.getElementById(
      ...
      <div class="cb-setting">
        <span class="cb-setting-name">${
          gesturefyController.l10n[`gf-floorp-${id}-settings-addons-id`]
        }</span>
        <p class="cb-setting-description">
          <span>${
            gesturefyController.l10n[
              `gf-floorp-${id}-settings-addons-id-description`
            ]
          }</span>
        </p>
        ${gesturefyController.commandSettings[id]?.UI()}
      </div>
      <button id="settingsSaveButton" type="submit">${
        gesturefyController.i18n.buttonSave
      }</button>
      <form>
    `);
      settingContentPage.getElementById("settingsForm").onsubmit =
        gesturefyController.commandSettings[id].submit;
      settingsContent.appendChild(settingContentPage);
      return templateFragment.firstElementChild;
    },
    settingsL10n: [
      "gf-floorp-open-extension-sidebar-settings-addons-id",
      "gf-floorp-open-extension-sidebar-settings-addons-id-description",
      "gf-floorp-open-extension-sidebar-settings-list-default",
      "gf-floorp-open-extension-sidebar-settings-list-unknwon",
    ],
    ...
    slideBack: function () {
      ...
      document.querySelector("#gesturePopupCommandSelect")._scrollPosition =
        commandsMain.scrollTop;

      const commandBarWrapper =
        gesturefyController.commandList.getElementById("commandBarWrapper");
      const currentPanel =
        gesturefyController.commandList.querySelector("#settingsPanel");

      const newPanel =
        gesturefyController.commandList.querySelector("#commandsPanel");
      ...
      currentPanel.addEventListener(
        "transitionend",
        function removePreviousPanel(event) {
    ...
    `);
      ...
      const settingsBackButton =
        templateFragment.getElementById("settingsBackButton");
    ...
    popupCommandSet(id, setting) {
      ...
      gesturefyController.commandList.children[1].textContent =
        gesturefyController.l10n["gf-floorp-" + id + "-name"];
      gesturefyController.commandList.children[1].setAttribute(
        "title",
        gesturefyController.l10n["gf-floorp-" + id + "-name"],
      );
      if (!(id in this.commandSettings)) {
        gesturefyController.commandList.children[2].classList.remove(
          "has-settings",
        );
      }
      document
        .querySelector("#gesturePopupLabelInput")
        .setAttribute(
          "placeholder",
          gesturefyController.l10n["gf-floorp-" + id + "-name"],
        );
      if (
        document
          .querySelector("#gesturePopupLabelInput")
          .getAttribute("FloorpGFBeforeText") ==
      ...
      if (
        ...
        if (
          ...
          if (
            document
              .querySelector("#gesturePopupLabelInput")
              .value.startsWith("[Floorp]")
          ) {
            document.querySelector("#gesturePopupLabelInput").value =
              gesturefyController.l10n["gf-floorp-open-extension-sidebar-name"];
          }
        }
        this.popupCommandSet(
          JSON.parse(valueJSON.settings.message).action,
          JSON.parse(valueJSON.settings.message).options,
        );
        if (
          this.l10n[
            "gf-floorp-" +
    ...
    clickCommandList(event) {
      if (
        event.currentTarget.id.replace("gf-floorp-", "") in
        gesturefyController.commandSettings
      ) {
        gesturefyController.showSettingPage(
          event.currentTarget.id.replace("gf-floorp-", ""),
        );
      } else {
        gesturefyController.closeCommandPanel(
          event.currentTarget.id.replace("gf-floorp-", ""),
        );
      }
    },
    closeCommandPanel(commandID, settings) {
      const commandBar =
        gesturefyController.commandList.getElementById("commandBar");
      ...
      commandBar.addEventListener(
        "transitionend",
        function removeCommandBar(event) {
          // prevent the event from firing for child transitions
          if (event.currentTarget === event.target) {
      ...
      document.querySelector("#gesturePopupCommandSelect")._selectedCommand =
        null;
      document.querySelector("#gesturePopupCommandSelect")._scrollPosition = 0;
    },
    commandMouseLeave(event) {
      const commandItemInfo =
        event.currentTarget.querySelector(".cb-command-info");
    ...
    commandMouseEnter(event) {
      ...
      setTimeout(() => {
        if (commandItem.matches(".cb-command-item:hover")) {
          ...
          if (!commandItemInfo.style.getPropertyValue("height")) {
            commandItemInfo.style.setProperty(
              "height",
              commandItemInfo.scrollHeight + "px",
            );
          }
        }
      }, 500);
    ...
    commandNameBlur() {
      if (
        ...
        document.querySelector("#gesturePopupLabelInput").value = document
          .querySelector("#gesturePopupLabelInput")
          .getAttribute("FloorpGFBeforeText");
        document.querySelector("#gesturePopupLabelInput").style.color = "gray";
      }
    ...
    setObserver() {
      if (document.querySelector("#gesturePopup") == null) {
        ...
        document
          .querySelector("#gesturePopupLabelInput")
          .addEventListener("focus", this.commandNameFocus);
        document
          .querySelector("#gesturePopupLabelInput")
          .addEventListener("blur", this.commandNameBlur);
      }
    },
    setCmdListObserver() {
      if (
        document.querySelector("#gesturePopupCommandSelect").shadowRoot == null
      ) {
        window.setTimeout(this.setCmdListObserver, 1000);
      } else {
        this.commandList = document.querySelector(
          "#gesturePopupCommandSelect",
        ).shadowRoot;
        this.cmdListObserver.observe(this.commandList, {
          childList: true,
          subtree: true,
        });
      }
    ...
    observerCommandSelectFunction() {
      ...
      if (
        this.commandList.querySelector(`[data-command="DuplicateTab"]`) !=
          null &&
      ...
      if (
        ...
        if (
          ...
          this.thereIsNotOverlay
        ) {
          this.commandList.querySelector("#settingsBackButton").click();
          if (
            JSON.parse(valueJSON.settings.message).action in
            gesturefyController.commandSettings
          ) {
            this.thereIsSetting = true;
          }
        } else if (
          ...
          this.commandList.querySelector(
            `.cb-active[data-command="SendMessageToOtherAddon"]`,
          )
        ) {
          this.commandList
            .querySelector(`.cb-active[data-command="SendMessageToOtherAddon"]`)
            .classList.remove("cb-active");
          this.commandList
            .querySelector(
              `#gf-floorp-${JSON.parse(valueJSON.settings.message).action}`,
            )
            .classList.add("cb-active");
        }
      } else {
        if (
          document
        ...
        document
          .querySelector("#gesturePopupLabelInput")
          .removeAttribute("FloorpGFBeforeText");
      }
      this.thereIsNotOverlay =
        this.commandList.querySelector("#overlay") == null;
    },
  };
  gesturefyController.observer = new MutationObserver(
    gesturefyController.observerFunction.bind(gesturefyController),
  );
  gesturefyController.cmdListObserver = new MutationObserver(
    gesturefyController.observerCommandSelectFunction.bind(gesturefyController),
  );
  ...
  (async () => {
    ...
    gesturefyController.extensionsInSidebar = await browser.runtime.sendMessage(
      { data: "", type: "extensionsInSidebar" },
    );
