...
function makeDynamicSheet(options) {
  ...
  let rules = `
    ...
    #navigator-toolbox {
      ...
      --paxmod-titlebar-display: ${
        options.displayTitlebar ? "-webkit-box" : "none"
      };
      --paxmod-titlebar-placeholders: ${
        options.displayPlaceholders ? "1000px" : "0px"
      };
      --paxmod-display-close-button: ${
        options.displayCloseButton ? "-webkit-box" : "none"
      };
      ${
        options.forceCompact &&
        "--proton-tab-block-margin: 0; --tab-block-margin: 0;"
      }
    }
    ${
      options.forceCompact &&
...
export async function applyOptions() {
  ...
  if (options.enableIconColors) {
    if (!browser.tabs.onUpdated.hasListener(onFavIconChanged)) {
      browser.tabs.onUpdated.addListener(onFavIconChanged, {
...
export function getBestLightnessOptions(theme) {
  ...
  for (let prop in invertColorMap) {
    ...
    return w3color(colors[prop]).isDark() !== invertColorMap[prop]
      ? dark
      : light;
  }
  return light;
...
(async () => {
  ...
  if (enabled == 1) {
    browser.theme.onUpdated.addListener(async (details) => {
      if (
        (await browser.storage.local.get("fitLightness")).fitLightness !== false
      ) {
