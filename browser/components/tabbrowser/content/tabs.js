...

{
	...
	class MozTabbrowserTabs extends MozElements.TabsBase {
		...
		init() {
			...
			this._tabDefaultMaxWidth = Number.NaN;
		...
		on_TabAttrModified(event) {
			if (
				["soundplaying", "muted", "activemedia-blocked", "sharing"].some(
					(attr) => event.detail.changed.includes(attr),
				)
			) {
		...
		on_transitionend(event) {
			...
			const tab = event.target ? event.target.closest("tab") : null;
			...
			const evt = new CustomEvent("TabAnimationEnd", { bubbles: true });
		...
		on_keydown(event) {
			...
			const [accel, nonAccel] =
				AppConstants.platform == "macosx"
					? [event.metaKey, event.ctrlKey]
					: [event.ctrlKey, event.metaKey];

			const keyComboForMove = accel && shiftKey && !altKey && !nonAccel;
			const keyComboForFocus = accel && !shiftKey && !altKey && !nonAccel;
			...
			const { visibleTabs, selectedTab } = gBrowser;
			const { arrowKeysShouldWrap } = this;
			let focusedTabIndex = this.ariaFocusedIndex;
			if (focusedTabIndex == -1) {
				focusedTabIndex = visibleTabs.indexOf(selectedTab);
			}
			const lastFocusedTabIndex = focusedTabIndex;
		...
		startTabDrag(event, tab, { fromTabList = false } = {}) {
			...
			const otherSelectedTabs = selectedTabs.filter(
				(selectedTab) => selectedTab != tab,
			);
			const dataTransferOrderedTabs = [tab].concat(otherSelectedTabs);

			const dt = event.dataTransfer;
			for (let i = 0; i < dataTransferOrderedTabs.length; i++) {
				const dtTab = dataTransferOrderedTabs[i];

				dt.mozSetDataAt(TAB_DROP_TYPE, dtTab, i);
				const dtBrowser = dtTab.linkedBrowser;
      ...
      const tabOffsetX = getClientElementPosition(tab) - getClientElementPosition(this);
      const tabOffsetY = getClientElementPosition(tab) - getClientElementPosition(this);
		...
		on_dragover(event) {
			...
			if (effects == "link") {
				const tab = this._getDragTargetTab(event, { ignoreTabSides: true });
				if (tab) {
					if (!this._dragTime) {
			...
			if (pixelsToScroll) {
				...
				const scrollRect = arrowScrollbox.scrollClientRect;
				...
				const newIndex = this._getDropIndex(event);
		...
		on_drop(event) {
			...
			if (draggedTab && dropEffect == "copy") {
				...
				const oldTranslateX = Math.round(draggedTab._dragData.translateX);
				const tabWidth = Math.round(draggedTab._dragData.tabWidth);
				const translateOffset = oldTranslateX % tabWidth;
				...
				if (oldTranslateX && oldTranslateX != newTranslateX && !(gReduceMotion) && !Services.prefs.getIntPref("floorp.tabbar.style") === 2) {
					for (const tab of movingTabs) {
						tab.toggleAttribute("tabdrop-samewindow", true);
						tab.style.transform = "translateX(" + newTranslateX + "px)";
						const postTransitionCleanup = () => {
						...
						if (gReduceMotion) {
							...
							const onTransitionEnd = (transitionendEvent) => {
								if (
				...
				const targetTab = this._getDragTargetTab(event, {
					ignoreTabSides: true,
				});
				const userContextId = this.selectedItem.getAttribute("usercontextid");
				const replace = !!targetTab;
				const newIndex = this._getDropIndex(event);
				const urls = links.map((link) => link.url);
				const csp = browserDragAndDrop.getCsp(event);
				...
				(async () => {
					if (
						...
						const answer = await OpenInTabsUtils.promiseConfirmOpenInTabs(
							urls.length,
							window,
						);
						if (!answer) {
		...
		on_dragend(event) {
			...
			if (eX > wX && eX < wX + window.outerWidth) {
				...
				const detachTabThresholdY =
					window.screenY + rect.top + 1.5 * rect.height;
			...
			const ourCssToDesktopScale =
				window.devicePixelRatio / window.desktopToDeviceScale;
			const screenCssToDesktopScale =
			...
			if (gBrowser.tabs.length == 1) {
				...
				const props = { screenX: left, screenY: top, suppressanimation: 1 };
		...
		get allTabs() {
			...
			const children = Array.from(this.arrowScrollbox.children);
			children.pop();
		...
		_initializeArrowScrollbox() {
			...
			arrowScrollbox.shadowRoot.addEventListener(
				...
				(event) => {
					...
					for (const tab of gBrowser._removingTabs) {
						gBrowser.removeTab(tab);
					}
		...
		observe(aSubject, aTopic) {
			switch (aTopic) {
				case "nsPref:changed":
					...
					for (const parent of [newTab, newTab2]) {
						if (!parent) {
							continue;
						}
		...
		_updateCloseButtons() {
			...
			window.requestAnimationFrame(() => {
				window.requestAnimationFrame(() => {
					...
					const tab = this._getVisibleTabs()[gBrowser._numPinnedTabs];
		...
		_handleTabSelect(aInstant) {
			const selectedTab = this.selectedItem;
			if (this.hasAttribute("overflow")) {
		...
		_lockTabSizing(aTab, aTabWidth) {
			...
			if (!this._tabDefaultMaxWidth) {
				this._tabDefaultMaxWidth = Number.parseFloat(
			...
			if (this.hasAttribute("overflow")) {
				...
				if (isEndTab) {
					const numNormalTabs = tabs.length - numPinned;
		...
		_expandSpacerBy(pixels) {
			const spacer = this._closingTabsSpacer;
			spacer.style.width =
				Number.parseFloat(spacer.style.width) + pixels + "px";
      ...
      const translate = verticalTabbarEnabled() ? "translateY" : "translateX";
			...
			const ltrMove = screenX > draggedTab._dragData[animLastScreen];
      draggedTab._dragData[animLastScreen] = screenX;

			const pinned = draggedTab.pinned;
			const numPinned = gBrowser._numPinnedTabs;
			...
			const tabWidth = draggedTab.getBoundingClientRect()[screenOrientation];
			const shiftWidth = tabWidth * movingTabs.length;
      ...
      draggedTab._dragData.tabHeight = tabWidth;
			...
			const rightMovingTabScreenX = movingTabs[movingTabs.length - 1][screen];;
			const leftMovingTabScreenX = movingTabs[0][screen];
			...
			const leftBound = leftTab[screen] - leftMovingTabScreenX;
			const rightBound =
				rightTab[screen] +
				rightTab.getBoundingClientRect()[screenOrientation] -
			...
			const leftTabCenter = leftMovingTabScreenX + translateX + tabWidth / 2;
			const rightTabCenter = rightMovingTabScreenX + translateX + tabWidth / 2;
			const tabCenter = ltrMove ? rightTabCenter : leftTabCenter;
			let newIndex = -1;
			const oldIndex =
				"animDropIndex" in draggedTab._dragData
					? draggedTab._dragData.animDropIndex
			...
			while (low <= high) {
				...
				if (screenX > tabCenter) {
					...
					screenX + tabs[mid].getBoundingClientRect()[screenOrientation] <
					tabCenter
				) {
					low = mid + 1;
				} else {
			...
			for (const tab of tabs) {
				if (tab != draggedTab) {
					const shift = getTabShift(tab, newIndex);
          // X,Y
		...
		_groupSelectedTabs(tab) {
			const draggedTabPos = tab._tPos;
			const selectedTabs = gBrowser.selectedTabs;
			const animate = !gReduceMotion;
			...
			for (let i = selectedTabs.indexOf(tab) - 1; i > -1; i--) {
				const movingTab = selectedTabs[i];
			...
			for (const t of this._getVisibleTabs()) {
				if (t.groupingTabsData && t.groupingTabsData.translateX) {
					const translateX = (RTL_UI ? -1 : 1) * t.groupingTabsData.translateX;
			...
			function addAnimationData(movingTab, movingTabNewIndex, side) {
				const movingTabOldIndex = movingTab._tPos;
				...
				const movingTabWidth = movingTab.getBoundingClientRect().width;
				const shift = (movingTabNewIndex - movingTabOldIndex) * movingTabWidth;
				...
				const lowerIndex = Math.min(movingTabOldIndex, draggedTabPos);
				const higherIndex = Math.max(movingTabOldIndex, draggedTabPos);

				for (let i = lowerIndex + 1; i < higherIndex; i++) {
					const middleTab = gBrowser.visibleTabs[i];
		...
		_finishGroupSelectedTabs(tab) {
			...
			const selectedTabs = gBrowser.selectedTabs;
			const tabIndex = selectedTabs.indexOf(tab);
		...
		handleEvent(aEvent) {
			switch (aEvent.type) {
				case "mouseout":
					...
					const relatedTarget = aEvent.relatedTarget;
		...
		_getDropIndex(event) {
			...
			const middle = tab.screenX + tab.getBoundingClientRect().width / 2;
			const isBeforeMiddle = RTL_UI
