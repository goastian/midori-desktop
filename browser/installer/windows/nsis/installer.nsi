...
Section "-Application" APP_IDX
  ...
  ${AddDisabledDDEHandlerValues} "MidoriPDF-$AppUserModelID" "$2" "$8,${IDI_DOCUMENT_PDF_ZERO_BASED}" \
...
!ifdef MOZ_DEFAULT_BROWSER_AGENT
  ...
  WriteRegDWORD HKCU "Software\Astian\${AppName}\Installer\$AppUserModelID" \
...
Section "-InstallEndCleanup"
  ...
  ${Unless} ${Silent}
    ...
    ${If} "$0" == "1"
      ...
      ${If} "$0" != ""
        ClearErrors
        WriteRegStr HKCU "Software\Astian\Midori" "OldDefaultBrowserCommand" "$0"
      ${EndIf}
    ...
    ${ElseIfNot} ${Errors}
      ...
      ClearErrors
      WriteRegStr HKCU "Software\Astian\Midori" "DefaultBrowserOptOut" "True"
      ${If} ${Errors}
...
Function SendPing
  ...
  ${If} $0 != ""
    ...
    ${Else}
      StrCpy $0 "$0" "" -11 # 11 == length of "midori.exe"
      ${If} "$0" == "${FileMainEXE}"
...
Function preComponents
  ...
  ClearErrors
  WriteRegStr HKLM "Software\Astian" \
...
!endif
  ...
  SetShellVarContext all
  ${GetFirstInstallPath} "Software\Astian\${BrandFullNameInternal}" $0
  ${If} "$0" == "false"
    SetShellVarContext current
    ${GetFirstInstallPath} "Software\Astian\${BrandFullNameInternal}" $0
    ${If} "$0" == "false"
